...
def _get_total_stopwords(frequencies: dict[Token: int]) -> int:
    ...
    for word, freq in frequencies.items():
        if word in STOPWORDS: total += freq
    return total


def has_high_textual_information_content(frequencies: dict[Token: int], min_word_threshold = 50, stopword_ratio_threshold = .5) -> bool:
    '''
